...
class ResultsPanel(JPanel):
    ...
    def __init__(self, callbacks, extender):
        ...
        # Store message_info by source for sending to Repeater
    ...
    def _init_ui(self):
        ...
        for title, key in categories:
            ...
            table.addMouseListener(TableDoubleClickListener(self))
    ...
    def register_message_info(self, source_name, message_info):
        """Store message_info for a source to enable sending to Repeater."""
        if source_name and message_info:
            self.message_info_map[source_name] = message_info

    def send_to_repeater(self, source_name):
        """Send the request for the given source to Repeater."""
        message_info = self.message_info_map.get(source_name)
        if not message_info:
            return False
        ...
        try:
            ...
            # Create a tab name from the source
            tab_name = source_name[:20] if len(source_name) > 20 else source_name
...
class TableDoubleClickListener(MouseAdapter):
    """Sends request to Repeater on double-click."""
    ...
    def mouseClicked(self, event):
        if event.getClickCount() == 2:
            ...
            row = table.getSelectedRow()
            if row >= 0:
                # Get the source name from column 1 (Source column)
                model_row = table.convertRowIndexToModel(row)
                source_name = str(table.getModel().getValueAt(model_row, 1))
                self.panel.send_to_repeater(source_name)
